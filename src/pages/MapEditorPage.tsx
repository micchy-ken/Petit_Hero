          ...
          <div className="bg-slate-700 rounded-xl border border-slate-500 shadow-2xl p-6 w-full max-w-sm flex flex-col gap-4 animate-in fade-in zoom-in-95">
            ...
            {editingEvent.type === 'teleport' && (
              <div className="flex flex-col gap-2">
                ...
                <label className="text-xs text-slate-300 font-bold uppercase mt-2">固有条件 (踏破率)</label>
                <select 
                  value={editingEvent.data?.requiredExplorationRate || 'null'}
                  onChange={(e) => setEditingEvent({ ...editingEvent, data: { ...editingEvent.data, requiredExplorationRate: e.target.value === 'null' ? undefined : Number(e.target.value) } })}
                  className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1.5 text-sm text-slate-200 outline-none"
                >
                  <option value="null">なし (条件なし)</option>
                  <option value="50">50%</option>
                ...
                <label className="text-xs text-slate-300 font-bold uppercase mt-2">固有条件 (捜索率)</label>
                <select 
                  value={editingEvent.data?.requiredSearchRate || 'null'}
                  onChange={(e) => setEditingEvent({ ...editingEvent, data: { ...editingEvent.data, requiredSearchRate: e.target.value === 'null' ? undefined : Number(e.target.value) } })}
                  className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1.5 text-sm text-slate-200 outline-none"
                >
                  <option value="null">なし (条件なし)</option>
                  <option value="50">50%</option>
                ...
                <label className="text-xs text-slate-300 font-bold uppercase mt-2">固有条件 (撃破率)</label>
                <select 
                  value={editingEvent.data?.requiredDefeatRate || 'null'}
                  onChange={(e) => setEditingEvent({ ...editingEvent, data: { ...editingEvent.data, requiredDefeatRate: e.target.value === 'null' ? undefined : Number(e.target.value) } })}
                  className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1.5 text-sm text-slate-200 outline-none"
                >
                  <option value="null">なし (条件なし)</option>
                  <option value="50">50%</option>
