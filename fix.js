...
// 壊れた部分を見つける
// 開始行: placeMode === 'event' ? 'bg-indigo-600 b                        onChange={(e) => {
// 終了行: 2つ目の </select> の後ろの </div> まで

const startIndex = code.indexOf(`                  <button 
                    onClick={() => setPlaceMode('event')}
                    className={\`flex items-center gap-2 w-full px-3 py-2 rounded text-sm transition-all \${
                      placeMode === 'event' ? 'bg-indigo-600 b`);

const endStr = `</optgroup>
                            <optgroup label="📦 通常登録アイテム">
                              {customItems.map((item) => (
                                <option key={item.id} value={item.id}>
                                  {getEquipmentIcon(item)} {item.name}
